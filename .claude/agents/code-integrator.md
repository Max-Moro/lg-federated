...
This is a **federated repository** with multiple subprojects:

```
lg/               → Repository root (you work here)
├── cli/          → Core engine (Python)
├── vscode/       → VS Code extension (TypeScript)
├── intellij/     → IntelliJ plugin (Kotlin)
└── lg-cfg/       → Configuration
```

**Important**: All file paths are relative to the federated root. When editing files, use full paths like:
- `vscode/src/services/MyService.ts`
- `intellij/src/main/kotlin/lg/intellij/services/MyService.kt`
...
1. Use Grep to find ALL usages across the relevant subproject
2. Update all call sites to match the new signature
3. Check for indirect dependencies (files importing the modified module)

**Example**: If changing `buildHtml()` from sync to async in VS Code extension, find all files in `vscode/` calling it and add `await`.
...
- Work across multiple subprojects (vscode/, intellij/) when instructed
...
Modified Files:
- vscode/src/services/file1.ts
- vscode/src/views/file2.ts
- intellij/src/main/kotlin/lg/intellij/services/file3.kt
- intellij/src/main/kotlin/lg/intellij/ui/file4.kt
...
- Any programming language (Python, TypeScript, Kotlin, Java, Go, Rust, etc.)
...
2. Parse instruction to identify:
   - Files to create/modify (may span vscode/ and intellij/)
